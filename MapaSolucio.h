...
public:
	...
	void getPdis(std::vector<PuntDeInteresBase*>& pdis) {
		...
		while (!pdis.empty()) pdis.pop_back();
	...
	void getCamins(std::vector<CamiBase*>& camins) {
		while (!camins.empty()) camins.pop_back();
	...
	void parsejaXmlElements(std::vector<XmlElement>& xmlElements) {
		...
		//CREEM UN VECTOR PER GUARDAR-HI NODES CAMI
		...
		for (auto it = xmlElements.begin(); it != xmlElements.end(); ++it) {
			...
			if ((*it).id_element == "node") {
				...
				for (int i = 0; i < (*it).atributs.size(); i++) {
					if ((*it).atributs[i].first == "lat") c.lat = std::stod((*it).atributs[i].second);
					else if ((*it).atributs[i].first == "lon") c.lon = std::stod((*it).atributs[i].second);
					else if ((*it).atributs[i].first == "id") id = (*it).atributs[i].second;
				}
				...
				for (int i = 0; i < (*it).fills.size() && !esCami; i++) {
					if ((*it).fills[i].first == "tag") {
						...
						if (tagValor.first == "highway" || tagValor.first == "public_transport" || tagValor.first == "entrance" || tagValor.first == "access") esCami = true;
						//SI T� NOM, EL GUARDEM
						else if (tagValor.first == "name") nom = tagValor.second;
						//PERMET WHEELCHAIR?
						else if (tagValor.first == "wheelchair") { if (tagValor.second == "yes") wheelchair = true; }
						//�S SHOP, RESTAURANT O RES? si �s shop opening_hours i tipus=tipus de shop, si �s restaurant tipus=tipus de cuisine
						else if (tagValor.first == "shop") { esShop = true; tipus = tagValor.second; }
						else if (tagValor.first == "amenity") { if (tagValor.second == "restaurant") esRestaurant = true; }
						else if (tagValor.first == "cuisine") tipus = tagValor.second;
						else if (tagValor.first == "opening_hours") opening_hours = tagValor.second;
					}
				}
				...
				else if (esShop) m_pdis.push_back(new PuntDeInteresBotigaSolucio(c, nom, tipus, opening_hours, wheelchair));
				else if (esRestaurant) m_pdis.push_back(new PuntDeInteresRestaurantSolucio(c, nom, tipus, wheelchair));
			}
				...
				for (int i = 0; i < (*it).fills.size() && !esHighway; i++) {
					if ((*it).fills[i].first == "tag") {
						std::pair<std::string, std::string> tagValor = Util::kvDeTag((*it).fills[i].second);
						if (tagValor.first == "highway") esHighway = true;
					}
				}
				if (esHighway) {
